...
def randomly_remove_fraction_of_elements(fraction, maze):
    # randomly remove 1/fraction the elements from  final maze to make it less 1-dimensional
    n_elements_to_remove = int(1/fraction*width*height)
...
# randomly_remove_fraction_of_elements(10, maze)

with open('new_maze.txt', 'w') as f:
    for row in range(len(maze)):
        for i, item in enumerate(maze[row]):
            f.write(item)
            if i != len(maze[row])-1:
                f.write(" ")
        f.write('\n')